...
def upsample_wav_train(model, wav, args, epoch = None, model_path = None):
    ...
    if fs == 16000:
        ...
        if epoch is not None:
            ...
            sf.write(outname_audio + '.r' + str(args.r) + '.lr.wav', x_lr, fs)
            sf.write(outname_audio + '.hr.wav', x_hr, fs)

            save_spectrum(get_spectrum(x_hr, n_fft=1 * 2048), sr=fs, hop_length=2048 // 4,
                          outfile=outname_spectrograms + '.hr.png', type='high resolution')

            save_spectrum(get_spectrum(x_lr, n_fft=1 * 2048), sr=fs, hop_length=1 * 2048 // 4,
                          outfile=outname_spectrograms + '.r' + str(args.r) +
                                  '.sr' + str(args.sr) + '.lr.png', type='low resolution')
    ...
    if fs == 48000:
        ...
        if epoch is not None:
            ...
            save_spectrum(get_spectrum(x_hr, n_fft=3 * 2048), sr=fs, hop_length=3*2048 // 4,
                          outfile=outname_spectrograms + '.hr.png', type='high resolution')

            save_spectrum(get_spectrum(x_lr, n_fft=3 * 2048), sr=fs, hop_length=3 * 2048 // 4,
                          outfile=outname_spectrograms + '.r' + str(args.r) +
                                  '.sr' + str(args.sr) + '.lr.png', type='low resolution')

            save_spectrum(get_spectrum(x_lr_1, n_fft=3 * 2048), sr=fs, hop_length=3 * 2048 // 4,
                          outfile=outname_spectrograms + '.r' + str(args.r) +
                                  '.sr' + str(args.sr) + '.spline.png', type='upsampled, spline')
        ...
        save_spectrum(get_spectrum(x_pr, n_fft=3*2048), sr= fs, hop_length=3*2048//4,
                  outfile=outname_spectrograms + '.r' + str(args.r) +
                          '.' + str(args.model) +
                          '.sr' + str(args.sr) + '.pr.png',
                      type = 'upsampled ' + str(args.model)  )
