...
def main():
    ...
    if (args.sr == '48000' and args.speaker == "multi"):
        ...
        file_name_gan_5 = "../results/MOS/scores_48_gan_alt_5.csv"
        file_name_gan_3 = "../results/MOS/scores_48_gan_alt_3.csv"

        plot_histogram(file_name_gan, fig_save_dir, args.sr,  file_name_gan_5, file_name_gan_3, file_name_adiounet )
        plot_skatter(file_name_gan, fig_save_dir, args.sr, file_name_adiounet)
...
def plot_histogram(file_name_gan, fig_save_dir, sr, file_name_gan_5, file_name_gan_3, file_name_adiounet):
...
##############################################
    ...
    ax_0.hist(MOS_pr, bins=fixed_bins, alpha=0.8, color="magenta", label="MU-GAN")
    ...
    #ax_2.hist(MOS_pr_gan_5, bins=fixed_bins, alpha=0.8, color="magenta", label="MU-GAN Alt 5")
    ...
    ax_2 = ax[1]
    ax_2.hist(MOS_pr_aunet, bins=fixed_bins, alpha=1.0, color="green", label="AudiouNet")
    ax_2.hist(MOS_pr_gan_3, bins=fixed_bins, alpha=0.8, color="magenta", label="MU-GAN Alt 3")
    ax_2.hist(MOS_hr, bins=fixed_bins, alpha=0.4, color="blue", label="hr")
    ...
    # Subplot 1
    print(len(SNR_pr_aunet), len( SNR_pr), len(SNR_pr_gan_5))

    ax_0 = ax[0]
    fixed_bins = np.linspace(0, 40, 41)  # 30 bins from 2 to 5
    ...
    # Subplot 3
    #print(np.histogram(SNR_pr_aunet, bins=fixed_bins), sum(np.histogram(SNR_pr_aunet, bins=30)[0]))
    #print(np.histogram(SNR_pr_gan_5, bins=fixed_bins), sum(np.histogram(SNR_pr_gan_5, bins=30)[0]))
######################################################
    ax_2 = ax[1]
...
#######################################################
    ax_2 = ax[2]
    ax_2.hist(SNR_pr_aunet, bins=fixed_bins, alpha=1.0, color="green", label="audiounet")
    ax_2.hist(SNR_pr_gan_3, bins=fixed_bins, alpha=0.8, color="magenta", label="GAN alt 3")
    # ax_2.hist(MOS_hr, bins=30, alpha=0.4, color="blue", label="hr")
