"""
Create an HDF5 file of raw audio patches for classifier training (48 kHz).
...
def parser_option():
  ...
  parser.add_argument('--sr', type=int, default=48000, help='audio sampling rate')
...
def add_data(h5_file, inputfiles, args):
  ...
  initial_rows = max(num_files * 2, 2)
  data_set = h5_file.create_dataset(
    'data', (initial_rows, d_lr, 1), maxshape=(None, d_lr, 1),
    dtype=np.float32, chunks=True
  )

  label_set = h5_file.create_dataset(
    'label', (initial_rows, 1, 1), maxshape=(None, 1, 1),
    dtype=np.float32, chunks=True
  )
  ...
  for j, file_path in enumerate(file_list):
    ...
    for i in range(0, len(x), s):
        ...
        if global_idx + 2 > data_set.shape[0]:
            new_rows = max(data_set.shape[0] * 2, global_idx + 2)
            data_set.resize(new_rows, axis=0)
            label_set.resize(new_rows, axis=0)
  ...
  print(f"Dataset written: {global_idx} rows", flush=True)
